...
def evaluate(
    ...
    if not item_entity_map:
        for i in range(model.item_size):
            entity_ids = torch.nonzero(model.entity_item_map.T[i]).squeeze().tolist()
            # 1要素しかないとintになる
    ...
    def process_single_user(user_data):
        """単一ユーザーのリランキング処理"""
        user, observed_items, pos_item, pred, reranker, k = user_data
        pred = pred.clone()  # コピーを作成してスレッドセーフに
        pred[observed_items] = -np.inf

        # リランキング
        candidates = torch.where(pred != -np.inf)[0].cpu().tolist()
        reranked_items = reranker.rerank(pred=pred, candidates=candidates, k=k)
        indices = torch.tensor(reranked_items)

        rank = np.where(indices == pos_item)[0][0] + 1
        ranking = indices.tolist()
        return int(user.cpu()) if hasattr(user, "cpu") else user, rank, ranking

    with torch.no_grad():
        for i in range(0, len(eval_data), batch_size):
            batch = eval_data[i : i + batch_size]
            batch_users = torch.tensor(
                [data[0] for data in batch], dtype=torch.long
            ).to(device)
            observed_items_list, pos_items = create_batch_items(batch, model.item_size)

            all_items = torch.tensor(list(range(model.item_size)), dtype=torch.long).to(
                device
            )
            ...
            if use_parallel:
                # 並列処理用のデータ準備
                user_data_list = [
                    (user, observed_items, pos_item, pred, reranker, k)
                ...
                with ThreadPoolExecutor(max_workers=n_workers) as executor:
                    futures = [
                        executor.submit(process_single_user, user_data)
                    ...
                    # 結果を追加
                    for user_id, rank, ranking in results:
                ...
                for user, observed_items, pos_item, pred in zip(
                    ...
                    user_id, rank, ranking = process_single_user(
                        (user, observed_items, pos_item, pred, reranker, k)
                    )
                    users.append(user_id)
                    ranks.append(rank)
...
def _category_coverage(
    ...
    for k in Ks:
        ...
        for ranking_list in ranking_lists:
            ...
            # ランキングの実際の長さとkの小さい方を使う
            actual_k = min(k, len(ranking_list))
...
def _display_metrics(mrr, hrs, ndcgs, diversities, ccs, log):
    ...
    log(f"{mrr:>7f}", *rounded_ndcgs, *rounded_diversities, *rounded_ccs, sep=",")
